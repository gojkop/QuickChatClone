...
async function listFeedback(req, res) {
  ...
  try {
    ...
    // Build separate queries for different sort fields (safest approach with postgres.js)
    let feedback;
    ...
    if (sortField === 'created_at' && sortDir === 'DESC') {
      ...
      // Use raw query for custom sorting (validated fields only)
      const whereConditions = ['f.deleted_at IS NULL'];
      const params = [];
...
async function createFeedback(req, res) {
 ...
 + (params.length + 1));
      ...
      if (journey_stage) {
        whereConditions.push('f.journey_stage = 
...
async function createFeedback(req, res) {
 ...
 + (params.length + 1));
      ...
      if (assigned_to === 'unassigned') {
        whereConditions.push('f.assigned_to IS NULL');
      } else if (assigned_to) {
...
async function createFeedback(req, res) {
 ...
 + (params.length + 1));
      ...
      if (has_email === 'true') {
        whereConditions.push('f.email IS NOT NULL');
      }
      if (date_from) {
...
async function createFeedback(req, res) {
 ...
 + (params.length + 1));
      ...
      if (search) {
        whereConditions.push('(f.message ILIKE 
...
async function createFeedback(req, res) {
 ...
 + (params.length + 1) + ' OR f.email ILIKE 
...
async function createFeedback(req, res) {
 ...
 + (params.length + 1) + ' OR f.page_url ILIKE 
...
async function createFeedback(req, res) {
 ...
 + (params.length + 1) + ')');
      ...
      feedback = await sql.unsafe(query, params);
    }
