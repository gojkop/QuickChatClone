import React from 'react';
import { Link } from 'react-router-dom';

function FinalCTA() {
  return (
    <section className="py-12 md:py-24 lg:py-32 bg-white relative overflow-hidden">
      {/* Subtle grid pattern */}
      <div className="absolute inset-0 bg-[linear-gradient(to_right,#8080800a_1px,transparent_1px),linear-gradient(to_bottom,#8080800a_1px,transparent_1px)] bg-[size:14px_24px]"></div>
      
      <div className="container mx-auto px-6 text-center relative z-10">
        <div className="max-w-4xl mx-auto">
          
          {/* ✅ PREMIUM: Enhanced headline with better typography */}
          <h2 
            className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-tight leading-tight"
            style={{
              letterSpacing: '-0.02em',
              fontFeatureSettings: '"kern" 1'
            }}
          >
            Where AI Stops,{' '}
            <span className="relative inline-block">
              <span className="bg-gradient-to-r from-indigo-600 to-violet-600 bg-clip-text text-transparent">
                Your Expertise Begins
              </span>
            </span>
          </h2>
          
          {/* ✅ PREMIUM: Enhanced subtext */}
          <p 
            className="mt-5 md:mt-6 max-w-2xl mx-auto text-lg md:text-xl text-gray-600 leading-relaxed"
            style={{
              letterSpacing: '0.01em',
              lineHeight: '1.6'
            }}
          >
            Turn your <span className="font-bold text-gray-900">last-mile expertise</span> into a revenue stream. 
            Get paid for the judgment, context, and accountability only you can provide—without adding meetings to your calendar.
          </p>
          
          {/* ✅ PREMIUM: Enhanced CTAs with micro-interactions */}
          <div className="mt-10 md:mt-12 flex flex-col sm:flex-row gap-4 justify-center items-center">
            <Link 
              to="/signin" 
              className="btn-premium btn-gradient-primary group inline-flex items-center gap-2 text-white font-bold py-4 px-8 md:py-5 md:px-10 rounded-xl text-base md:text-lg transition-all"
              style={{
                boxShadow: '0 4px 12px -2px rgba(99, 102, 241, 0.3), 0 2px 6px -1px rgba(139, 92, 246, 0.2), inset 0 1px 0 0 rgba(255, 255, 255, 0.2)'
              }}
            >
              <span>Claim My Free Expert Link</span>
              <svg className="w-5 h-5 group-hover:translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M17 8l4 4m0 0l-4 4m4-4H3" />
              </svg>
            </Link>
            
            <Link 
              to="/pricing" 
              className="btn-premium inline-flex items-center gap-2 text-gray-700 font-semibold py-4 px-6 md:py-5 md:px-8 rounded-xl border-2 border-gray-300 hover:border-indigo-600 hover:bg-indigo-50 transition-all bg-white"
              style={{
                boxShadow: '0 1px 3px 0 rgba(0, 0, 0, 0.04), inset 0 1px 0 0 rgba(255, 255, 255, 0.9)'
              }}
            >
              View Pricing
            </Link>
          </div>
          
          {/* Trust signals with enhanced styling */}
          <div className="mt-12 md:mt-16 flex flex-wrap justify-center items-center gap-6 text-sm text-gray-500">
            {[
              { text: 'No credit card required' },
              { text: '5-minute setup' },
              { text: 'Cancel anytime' }
            ].map((item, index) => (
              <div key={index} className="flex items-center gap-2" style={{ letterSpacing: '0.01em' }}>
                <svg className="w-5 h-5 text-green-500" fill="currentColor" viewBox="0 0 20 20">
                  <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
                </svg>
                <span className="font-medium">{item.text}</span>
              </div>
            ))}
          </div>
        </div>
      </div>
    </section>
  );
}

export default FinalCTA;