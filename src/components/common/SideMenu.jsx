...
function SideMenu({ isOpen, onClose, userInfo, isLoadingProfile }) {
  ...
  const handleCopyProfileLink = () => {
    // Check if userInfo and handle exist before copying
    if (userInfo && userInfo.handle) {
      const profileUrl = `${window.location.origin}/u/${userInfo.handle}`;
      navigator.clipboard.writeText(profileUrl);
      // You could add a toast notification here for better user experience
    }
    onClose();
  };
  ...
  // Dynamically generate account items only when userInfo is available
  const accountItems = userInfo && userInfo.handle ? [
    ...
    { to: `/u/${userInfo.handle}`, label: 'View Public Profile', icon: <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" /></svg> },
  ] : [];
    ...
    <div className={`fixed inset-0 z-50 transition-all duration-300 ease-out ${isOpen ? '' : 'pointer-events-none'}`}>
      ...
      <aside className={`relative h-full w-80 max-w-[85vw] bg-white shadow-2xl flex flex-col transform transition-transform duration-300 ease-out ${isOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex-1 overflow-y-auto px-4 py-6">
          ...
          {isAuthenticated && !isLoadingProfile && userInfo && (
            <div className="mb-6">
              <div className="px-3 mb-2 text-xs font-bold text-gray-400 uppercase tracking-wider">
                My Account
              </div>
