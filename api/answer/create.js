...
// Works with existing schema: answer.media_asset_id references media_asset table
...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Extract review_token from answer (generated by Xano)
    const reviewToken = answer.review_token;

    if (!reviewToken) {
      console.warn('⚠️ No review_token in answer response - check Xano endpoint');
    }

    // TODO: Send notification email to question asker
    // await sendAnswerNotificationEmail(answer, reviewToken);
    ...
    return res.status(201).json({
      ...
      data: {
        ...
        review_url: reviewToken ? `${process.env.NEXT_PUBLIC_BASE_URL}/r/${reviewToken}` : null,
