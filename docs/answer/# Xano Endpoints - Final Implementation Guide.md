...
sqlALTER TABLE answer ADD COLUMN text_response TEXT;
...
✅ Step 4: Update POST /answer Endpoint (10 minutes)
Modify your answer creation to work with the new fields and update media_asset.
Function Stack
1. Validate Input
...
3. Add Record to answer table
   ...
   - media_asset_id: input.media_asset_id (can be null)
...
5. Update media_asset owner (if media exists)
   IF input.media_asset_id THEN
     UPDATE media_asset
     SET owner_id = var.created_answer.id
     WHERE id = input.media_asset_id
   END IF
...
✅ Step 5: Create POST /media_asset (Already Exists?)
...
Function Stack
1. Add Record to media_asset
   Fields:
   - owner_type: input.owner_type
   - owner_id: input.owner_id (can be null, will be updated later)
...
🧪 Testing Sequence
Test in this order:
1. Test Media Asset Creation
bashcurl -X POST {XANO_URL}/media_asset \
...
Expected: Returns media_asset with id
2. Test Answer Creation
bashcurl -X POST {XANO_URL}/answer \
  -H "Authorization: Bearer YOUR_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "question_id": 1,
...
bashcurl {XANO_URL}/review/{playback_token_hash}
Expected: Returns question + answer with all fields
4. Test Feedback Submission (Public)
bashcurl -X POST {XANO_URL}/review/{playback_token_hash}/feedback \
  -H "Content-Type: application/json" \
  -d '{
    "rating": 5,
...
Xano Endpoints
 ...
 /answer (POST) updated to return playback_token_hash
 /media_asset (POST) exists and working

Testing

 Can create media_asset
 Can create answer with media
...
Expert records answer in AnswerRecorder
Frontend concatenates segments and uploads to Cloudflare
Frontend creates media_asset via /api/media/create-asset
Frontend creates answer via /api/answer/create
Xano updates question status and media_asset owner
Expert sees success with review link
Asker receives email with link to /r/{playback_token_hash}
Asker views answer at public review page
Asker submits feedback (rating + text)
Expert sees feedback in their dashboard
...
✅ You're Ready!
All frontend code is updated and ready. Once you complete these 5 Xano steps, the entire answer system will work end-to-end!
Estimated Time: 30-45 minutes for all Xano changes
Next Steps:

Add database columns (2 min)
Update /review/{token} endpoint (5 min)
Create /review/{token}/feedback endpoint (10 min)
Update /answer endpoint (10 min)
Verify /media_asset endpoint exists (5 min)
